...
def index(skicall):
    ...
    headersection['title', 'large_text'] = 'AlertClear1'
    # A textblock contains the widget description
    ref = "widgets.confirm.AlertClear1"
...
def show_widget(skicall):
    "Called by a button to show the AlertClear1"
    ...
    pd = PageData()
    pd['alertclear1', 'para_text'] = "Text set into the widget by JSON"
    pd['alertclear1', 'hide'] = False
    skicall.update(pd)


def show_error(skicall):
    "Called by a button to raise an error on the AlertClear1"
    raise FailPage(message="An Error has been raised", widget="alertclear1")
