...
def index(skicall):
    ...
    headersection['title', 'large_text'] = 'ShowResponders'
    # A textblock contains the widget description
    ref = "widgets.debug_tools.ShowResponders"
...
def respond(skicall):
    ...
    if ('settext', 'input_text') not in skicall.call_data:
        raise FailPage(message="No submission received")

    value = skicall.call_data['settext', 'input_text']
    pd = PageData()
