...
def index(skicall):
    ...
    headersection['title', 'large_text'] = 'HeaderText1'
    # A textblock contains the widget description
    ref = "widgets.headers.HeaderText1"
...
def settext(skicall):
    """Responds to submission from the SubmitTextInput4 widget form.
       ...
       target so the headertext1 widget is updated via a JSON responce,
    ...
    pd = PageData()
    pd['headertext1','large_text'] = skicall.call_data["setheadertext","input_text"]
    pd['headertext1','small_text'] = "And text can also be added here: " + skicall.call_data["setheadertext","input_text"]
    skicall.update(pd)
