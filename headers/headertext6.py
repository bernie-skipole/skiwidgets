...
def index(skicall):
    ...
    headersection['title', 'large_text'] = 'HeaderText6'
    # A textblock contains the widget description
    ref = "widgets.headers.HeaderText6"
...
def settext(skicall):
    """Responds to submission from the SubmitTextInput4 widget form.
       ...
       target so the headertext6 widget is updated via a JSON responce,
    ...
    pd = PageData()
    pd['headertext6','large_text'] = skicall.call_data["setheadertext","input_text"]
    pd['headertext6','small_text'] = "And text can also be added here: " + skicall.call_data["setheadertext","input_text"]
    skicall.update(pd)
