...
def index(skicall):
    ...
    headersection['title', 'large_text'] = 'HeaderText4'
    # A textblock contains the widget description
    ref = "widgets.headers.HeaderText4"
...
def settext(skicall):
    """Responds to submission from the SubmitTextInput4 widget form.
       ...
       target so the headertext4 widget is updated via a JSON responce,
    ...
    pd = PageData()
    pd['headertext4','large_text'] = skicall.call_data["setheadertext","input_text"]
    pd['headertext4','small_text'] = "And text can also be added here: " + skicall.call_data["setheadertext","input_text"]
    skicall.update(pd)
