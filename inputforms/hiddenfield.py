...
def index(skicall):
    ...
    headersection['title', 'large_text'] = 'HiddenField'
    # A textblock contains the widget description
    ref = "widgets.inputforms.HiddenField"
...
def submit_fields(skicall):
    ...
    if ('hiddenfield', 'hidden_field') not in skicall.call_data:
        raise FailPage(message="No submission received")

    pd = PageData()
    pd['result','para_text'] = "Text Received : " + skicall.call_data['textinput2', 'input_text']
    pd['result2','para_text'] = "Hidden Text Received : " + skicall.call_data['hiddenfield', 'hidden_field']
