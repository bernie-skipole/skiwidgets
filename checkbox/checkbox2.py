...
def index(skicall):
    ...
    headersection['title', 'large_text'] = 'CheckBox2'
    # A textblock contains the widget description
    ref = "widgets.checkbox.CheckBox2"
...
def respond(skicall):
    """Responds to submission from the CheckBox2 widget.
    ...
    if ('checkbox2', 'checkbox') not in skicall.call_data:
        raise FailPage(message="No CheckBox2 submission received")

    pd = PageData()
    if skicall.call_data['checkbox2', 'checkbox'] == 'checkbox2set':
        # The checkbox is ticked
        pd['result','para_text'] = "The CheckBox2 is ticked"
    else:
        # The checkbox is not ticked
        pd['result','para_text'] = "The CheckBox2 is not ticked"
    skicall.update(pd)
